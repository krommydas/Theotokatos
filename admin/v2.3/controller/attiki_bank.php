...
class ControllerPaymentAttikiBank extends Controller {
    ...
    protected function validate() {
    	...
    	$found_supported_currency = false;;
    	foreach($this->model_localisation_currency->getCurrencies() as $currency)
    	{
    		if($currency['status'] != $enabled_currency_status || !in_array($currency['code'], $this->supported_currencies))
    			continue;
    			
    		$found_supported_currency = true;
    		break;
    	}
    
    	if (!$found_supported_currency)
    ...
    private function validateOrderStatusesEditData()
    {
    	...
    	if(count(array_count_values($statuses)) < count($this->order_statuses))
    ...
    private $order_statuses = array('succeeded');
